<div class="bg-blue-700 px-4 sm:px-6 py-3 flex items-center justify-between shadow h-16 fixed top-0 left-0 right-0 z-50">
    <div class="flex items-center text-white">
        <img src="/images/logo.png" alt="" style="width: 6%; height: 6%; margin-right: 3%;">
    ...
    <div class="relative">
        @auth
            <div class="flex items-center cursor-pointer" id="opennavdropdown">       
                @role('Admin')
                <img class="w-8 h-8 rounded-full mr-2" src="/images/profile/user.png" >
                @else
                <img class="w-8 h-8 rounded-full mr-2" src="/storage/profile_pictures/{{auth()->user()->profile_picture }}" >
                @endrole  
                <p class="text-white">{{auth()->user()->name}}</p>         
                <p class="text-sm text-white font-semibold leading-none"></p>
            ...
            <div class="bg-blue-700 absolute top-0 right-0 mt-12 -mr-6 shadow rounded-bl rounded-br">
                <div class="hidden h-12 w-48" id="navdropdown">
        ...
        @else 
            <div class="flex items-center">
                @if (Route::has('login'))
                    <div>
                        <a class="flex items-center mr-4 text-sm text-gray-200 font-semibold" href="">
